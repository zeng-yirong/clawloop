...
def apply_nanoclaw_positive_advantage_bad_turn_mask(batch: DataProto) -> dict[str, float]:
    ...
    Rollout-side detection always records candidate spans, independently of
    whether masking is enabled. This keeps candidate/masked metrics comparable
    across mask-on and mask-off experiments. When positive-advantage masking is
    enabled, only positive-advantage tokens from enabled candidate spans are
    removed from the response mask.
    """
    metrics = {
    ...
    if "advantages" not in batch.batch or "response_mask" not in batch.batch:
        return {}
    if bad_turn_spans is None:
        return metrics

    apply_mask = bool_env("NANOCLAW_MASK_ONLY_POSITIVE_ADVANTAGE", False)
    ...
    for row_index, spans in enumerate(bad_turn_spans):
        if not isinstance(spans, (list, tuple)):
            continue
        merged_spans: dict[tuple[int, int], dict[str, Any]] = {}
        for span in spans:
            ...
            if end <= start:
                continue
            key = (start, end)
            merged = merged_spans.setdefault(key, {"mask_enabled": False, "token_count": 0})
            merged["mask_enabled"] = bool(merged["mask_enabled"] or span.get("mask_enabled", True))
            try:
                merged["token_count"] = max(int(merged["token_count"]), int(span.get("token_count", 0)))
            except (TypeError, ValueError):
                pass

        for (start, end), span_state in merged_spans.items():
            current_mask = response_mask[row_index, start:end]
            active_mask = current_mask.bool()
            active_tokens = int(active_mask.sum().detach().item())
            recorded_tokens = int(span_state.get("token_count", 0))
            candidate_token_count = max(active_tokens, recorded_tokens)
            if candidate_token_count <= 0:
                continue
            candidate_turns += 1
            candidate_tokens += candidate_token_count
            if not apply_mask or not bool(span_state.get("mask_enabled", True)):
                continue
